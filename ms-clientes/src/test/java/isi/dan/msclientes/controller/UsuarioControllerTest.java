...
public class UsuarioControllerTest {
    ...
    @Test
    void testGetClientesDelUsuario() throws Exception {
        List<Cliente> clientes = Arrays.asList(cliente);
        Mockito.when(usuarioService.getClientesDelUsuario(1)).thenReturn(clientes);

        mockMvc.perform(get("/api/usuarios/1/cliente"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$[0].nombre").value("Empresa Test"))
